...
namespace RiderAspNetMvc.Migrations
{
    ...
    internal sealed class Configuration : DbMigrationsConfiguration<DataAccess.AppDbContext>
    {
        ...
        private static void SeedAll(DataAccess.AppDbContext db, List<string> log)
        {
            var firstNames = new[] {
            ...
            var skip = Math.Ceiling(lnLength / 4.0) + 1;
            ...
            for (var studentIndex = 0; studentIndex < students.Length; studentIndex++)
            {
                ...
                log.Add($"{studentIndex}/{students?.Length} ({ student?.Name })   " +
                    $"{courseIndex}/{courses?.Length} ({ course?.Name })");

                if (student == null) log.Add("Student is null");
                if (course == null) log.Add("Course is null");
            ...
            log.Add(
                string.Join("\n\n",
                    ...
                    string.Join("\n", enrollments.Select(e => $"{e.CourseId}:{e.StudentId}")),
